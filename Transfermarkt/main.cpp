...
using namespace std;
void get_status(Persoana * p)
{
    auto p_new1= dynamic_cast<Jucator *>(p);
    if(p_new1)
        cout<<"Jucator - ";
    auto p_new2= dynamic_cast<Antrenor *>(p);
    if(p_new2)
        cout<<"Antrenor - ";
}
int main() {
    ...
    Persoana* p1 = &jucator1;
    Jucator jucator2("Dembele", "Ousmane", 25, "Barcelona", 50);
    Jucator jucator3("Lewandowski", "Robert", 34, "Barcelona", 40);
    //Creez jucatorii

    Antrenor antrenor1("Xavi", "Hernandez", 44, "4-3-3");
    Persoana* p2 = &antrenor1;
    //Creez antrenorii
    ...
    vector<Ikit*> kituri;
    kituri.push_back(new Kit("rosu","albastru"));
